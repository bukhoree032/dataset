...
@section('app-content')
    <div class="tile">
        <div class="tile-title-w-btn">
            ...
            <p>
                <!-- <a class="btn btn-primary" href="{{ route('admin.household.store.create') }}"><i class="fa fa-plus-circle fa-fw"></i>เพิ่มสมาชิก</a> -->
                ...
                {{--<a class="btn btn-outline-success" href="{{ route('admin.household.index') }}{{ $query_string }}"><i class="fas fa-file-pdf fa-fw"></i>ส่งออกรายงาน (*.PDF) </a>--}}
                                ...
                                <div class="option-link">
                                        <div class="btn-group btn-group-sm" role="group">
                                            ...
                                            @isset($value->householdMemberGeneral->id)
                                                <a class="btn btn-sm btn-info" href="{{ route('admin.household.info.member.step1_edit',[$store_id, $info_id, $value->id, $value->householdMemberGeneral->id]) }}" data-toggle="tooltip">
                                                ข้อมูลส่วนที่ 1</a>
                                            @else
                                                <a href="{{ route('admin.household.info.member.step1',[$store_id, $info_id, $value->id]) }}" class="btn btn-sm btn-info">ข้อมูลส่วนที่ 1</a>
                                            @endisset
                                            <div class="btn-group btn-group-sm" role="group">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                        @isset($value->householdMemberHealth->id)
                                                            <a class="dropdown-item" href="{{ route('admin.household.info.member.step2_edit',[$store_id, $info_id, $value->id, $value->householdMemberHealth->id]) }}"><i class="fas fa-heartbeat"></i> ข้อมูลส่วนที่ 2</a>
                                                        @else
                                                            <a class="dropdown-item" href="{{ route('admin.household.info.member.step2',[$store_id, $info_id, $value->id]) }}"><i class="fas fa-heartbeat"></i> ข้อมูลส่วนที่ 2</a>
                                                        @endisset
                                        ...
                                        @csrf
                                        <a href="{{ route('admin.household.info.member.edit',[$store_id, $info_id, $value->id]) }}" class="btn btn-sm btn-info">แก้ไข</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
        ...
        <div class="tile-footer">
            <a href="{{ route('admin.household.info.econ.create',[$store_id, $info_id]) }}" class="btn btn-secondary">ข้อมูลครัวเรือน ส่วนที่ 3</a>
            <a href="{{ route('admin.household.info.enviro.create',[$store_id, $info_id]) }}" class="btn btn-secondary">ข้อมูลด้านสิ่งแวดล้อม ส่วนที่ 4</a>
            <a href="{{ route('admin.household.info.political.create',[$store_id, $info_id]) }}" class="btn btn-secondary">ข้อมูลด้านการเมืองการปกครอง ส่วนที่ 5</a>
            <a href="{{ route('admin.household.info.communicat.create',[$store_id, $info_id]) }}" class="btn btn-secondary">ข้อมูลด้านการสื่อสารภายในครัวเรือน ส่วนที่ 6</a>
        </div>
    </div>

    <form action="{{ route('admin.household.info.member.store', [$store_id, $info_id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
